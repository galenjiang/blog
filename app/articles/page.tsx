...
export default async function Page() {
    ...
    return <div className="container mx-auto min-h-[calc(100vh-100px)]">
        ...
        <main className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {
                list.map(article => (<Link key={article.name} href={`articles/${article.name}`} className="flex flex-col rounded-2xl bg-white dark:bg-gray-800 shadow hover:shadow-lg px-8 py-6 aspect-auto transition">
